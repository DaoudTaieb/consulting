...
const filteredFournisseurs = computed(() => {
    if (!searchQuery.value) return props.fournisseurs;
    const q = searchQuery.value.toLowerCase();
    return props.fournisseurs.filter(f => {
        const code = String(f.fournisseurcode ?? f.fournisseurid).toLowerCase();
        const nom = String(f.nom || '').toLowerCase();
        const solde = String(f.solde || '').toLowerCase();
        return code.includes(q) || nom.includes(q) || solde.includes(q);
    });
});
        ...
        <div class="p-4 sm:p-6 md:p-8 lg:p-12 pb-8 safe-bottom">
            <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
                ...
                <div class="flex flex-wrap items-center gap-3 self-start md:self-auto">
                    <Link href="/fournisseurs/releves-globaux" class="bg-brand-charcoal text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-brand-gold transition-colors shadow-sm flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                        Liste des relevés
                    </Link>
                    <div class="px-4 py-2.5 bg-white border border-[#e3e3e0] rounded-xl text-sm font-bold text-[#1b1b18] shadow-sm">
            ...
            <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-[#e3e3e0] shadow-sm overflow-hidden">
                <div class="p-6 border-b border-[#f0f0f0] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="relative w-full sm:w-auto">
                        ...
                        <input v-model="searchQuery" type="text" placeholder="Rechercher..." class="w-full bg-[#f8f8f7] border-transparent rounded-xl py-2 pl-10 pr-4 text-sm focus:bg-white focus:border-brand-gold focus:ring-0 transition-all outline-none sm:w-64" />
                    </div>
                </div>
                ...
                <div class="overflow-x-auto table-scroll custom-scrollbar">
                    <table class="w-full text-left min-w-[500px]">
                        ...
                        <tbody class="divide-y divide-[#f0f0f0]">
                            <tr v-for="f in filteredFournisseurs" :key="f.fournisseurid" class="hover:bg-[#fcfcfb] group transition-colors">
