...
const filteredClients = computed(() => {
    ...
    return props.clients.filter(c => {
        ...
        const code = String(c.clientcode ?? c.clientid).toLowerCase();
        const solde = String(c.solde || '').toLowerCase();
        return nom.includes(query) || code.includes(query) || solde.includes(query);
    });
});
