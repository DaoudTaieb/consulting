    ...
    <MainLayout>
        ...
        <div class="p-4 sm:p-6 lg:p-6 pb-8 safe-bottom max-w-[1920px] mx-auto">
            ...
            <div v-show="activeTab === 'journee'" class="space-y-8">
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead class="sticky top-0 bg-white shadow-sm z-10">
                                    ...
                                    <tr class="bg-brand-gold/5 font-black text-brand-gold">
                                        ...
                                        <td class="px-2 py-2 text-right">{{ parseFloat(chiffreAffaireFamille?.reduce((a,b)=>a+Number(b.qte),0).toFixed(3)) }}</td>
                                ...
                                <tbody class="divide-y divide-[#f5f5f4] text-[11px]">
                                    <tr v-for="(f, i) in chiffreAffaireFamille" :key="i" class="hover:bg-[#fcfcfb]">
                                        ...
                                        <td class="px-2 py-2 text-right font-bold">{{ parseFloat(Number(f.qte).toFixed(3)) }}</td>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead class="sticky top-0 bg-white shadow-sm z-10">
                                    ...
                                    <tr class="bg-brand-gold/5 font-black text-brand-gold">
                                        ...
                                        <td class="px-2 py-2 text-right">{{ parseFloat(ventesRubrique?.reduce((a,b)=>a+Number(b.qte),0).toFixed(3)) }}</td>
                                ...
                                <tbody class="divide-y divide-[#f5f5f4] text-[10px]">
                                    <tr v-for="(v, i) in ventesRubrique" :key="i" class="hover:bg-[#fcfcfb]">
                                        ...
                                        <td class="px-2 py-2 text-right font-bold">{{ parseFloat(Number(v.qte).toFixed(3)) }}</td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-[2.5rem] border border-[#e3e3e0] shadow-sm overflow-hidden pb-4">
                        ...
                        <div class="p-2 overflow-x-auto custom-scrollbar">
            ...
            <div v-show="activeTab === 'mensuel'" class="space-y-6">
                ...
                <div class="bg-white p-4 sm:p-6 rounded-[2rem] border border-[#e3e3e0] shadow-sm">
                    <div class="grid grid-cols-2 gap-3 sm:gap-4 items-end lg:grid-cols-[1fr_1fr_1.2fr_auto]">
                        <div class="filter-field col-span-1">
                            <label class="filter-label">DU</label>
                            <input v-model="from_month" type="month" class="filter-input" />
                        </div>

                        <div class="filter-field col-span-1">
                            <label class="filter-label">AU</label>
                            <input v-model="to_month" type="month" class="filter-input" />
                        </div>

                        <div class="filter-field col-span-2 lg:col-span-1">
                            <label class="filter-label">SITE</label>
                            <select v-model="site_id" class="filter-input">
                                <option value="">Tous les sites</option>
                                <option v-for="s in sites" :key="s.siteid" :value="s.siteid">{{ s.libelle }}</option>
                            </select>
                        </div>
                        ...
                        <div class="col-span-2 lg:col-span-1 flex justify-end">
                            <button
                                ...
                                class="bg-brand-gold text-white px-5 py-3 rounded-2xl hover:opacity-90 shadow-lg shadow-brand-gold/20 transition-all active:scale-95 inline-flex items-center justify-center gap-2 font-black text-sm w-full sm:w-auto"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                                </svg>
                ...
                <div class="bg-white rounded-[2rem] border border-[#e3e3e0] shadow-sm overflow-hidden">
                    <div class="table-scroll custom-scrollbar table-scroll-hint touch-pan-x overscroll-x-contain">
                        <table class="w-full text-left min-w-[900px] whitespace-nowrap border-collapse">
                            <thead class="sticky top-0 bg-white shadow-sm z-10">
