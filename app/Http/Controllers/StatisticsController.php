...
class StatisticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        while ($current->lte($end)) {
            ...
            // --- CA VENTE ---
            $vente_cbls = DB::table('cbls')->whereBetween('cbldate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->where('transfere', false)->sum('netapayer');
            $vente_cfactures = DB::table('cfactures')->whereBetween('cfacturedate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            $vente_ctickets = DB::table('ctickets')->whereBetween('cticketdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->where('transfere', false)->sum('netapayer');
            $vente_cbrs = DB::table('cbrs')->whereBetween('cbrdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->where('transfere', false)->sum('netapayer');
            $vente_cavoirs = DB::table('cavoirs')->whereBetween('cavoirdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            
            $vente = $vente_cbls + $vente_cfactures + $vente_ctickets - $vente_cbrs - $vente_cavoirs;

            // --- CA ACHAT ---
            $achat_fbls = DB::table('fbls')->whereBetween('fbldate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->where('transfere', false)->sum('netapayer');
            $achat_ffactures = DB::table('ffactures')->whereBetween('ffacturedate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            $achat_fbrs = DB::table('fbrs')->whereBetween('fbrdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->where('transfere', false)->sum('netapayer');
            $achat_favoirs = DB::table('favoirs')->whereBetween('favoirdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            
            $achat = $achat_fbls + $achat_ffactures - $achat_fbrs - $achat_favoirs;
            ...
            // --- DIFFERENCE ---
            // différence = netapayer cbls + netapayer cfactures - netapayer cbrs - netapayer cavoirs -netapayer fbls - netapayer ffactures + netapayer fbrs + netapayer favoirs + reglemnt clt - reglement frs - depenses
            $diff_cbls = DB::table('cbls')->whereBetween('cbldate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            $diff_cbrs = DB::table('cbrs')->whereBetween('cbrdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            $diff_fbls = DB::table('fbls')->whereBetween('fbldate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            $diff_fbrs = DB::table('fbrs')->whereBetween('fbrdate', [$mStart, $mEnd])->when($siteId, fn($q) => $q->where('siteid', $siteId))->sum('netapayer');
            
            $resultat = $diff_cbls + $vente_cfactures - $diff_cbrs - $vente_cavoirs - $diff_fbls - $achat_ffactures + $diff_fbrs + $achat_favoirs + $reg_clt - $reg_frs - $depense;
